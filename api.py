...
try:
    ...
    _logging.warning(f"Could not import monitoring: {e}")
...
def get_db_connection(timeout=30.0):
    """
    Create a database connection: use Postgres pool if available, else fallback to SQLite.
    NOTE: For context manager usage, use database.get_db_connection() instead.
    """
    db_url = os.environ.get('DATABASE_URL')
    if db_url:
        # Try to get from pool first, fallback to direct connection
        try:
